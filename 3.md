...
This data structure, a series of "key" : "value" pairs, look a lot like a dictionary, which is a data structure we coded in Lesson 1. In fact it *is* a dictionary. Every point count stationID has a key:value pair, representing each landcover class :number of pixels in that landcover class, for every buffer in the dataset. </p>
