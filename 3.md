...
<code>print("landcover_pixel_counts:", landcover_pixel_counts);</code></p>
  
<p>If you look at the structure of landcover_pixel_counts, you'll notice that that under features->0 (or any other band)->properties->histogram you'll see 11: 71; 21: 2001.513725490196; and so on. We're looking at data in the 0th band. Each band represents one buffer surrounding one point count datapoint. The numbers on the left-hand side represent our National Landcover Database landcover classes, while the numbers on the right-hand side represent the number of pixels per landcover class. This data structure, a series of "key" : "value" pairs, look a lot like a dictionary, which is a data structure we coded in Lesson 1. Every point count stationID has a key:value pair, representing each landcover class :number of pixels in that landcover class, for every buffer in the dataset. </p>
