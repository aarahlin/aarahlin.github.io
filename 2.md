...
<p>Dates have to be written as YYYY-MM-DD in Google Earth Engine, and **include** the starting date, but **do not include** the ending date. Just another quirk of GEE to keep in mind when you're filtering your image data. </p>
...
<p>Look at the documentation of the Landset data collection to examine the bands in the dataset. Do this by clicking the dataset link at the top of your console. Notice that CLOUD_COVER is the name of one of the bands. To avoid cloudy images, we can use metadata that comes with each image. The <code>'CLOUD_COVER'</code> property tells you what percent of the image is cloudy. Use <code>ee.Filter.lt()</code> to keep only images where cloud cover is less than a certain threshold, say, 10%. This is a pretty typical cutoff for filtering satellite data </p>
<code>mayImages.filter(ee.Filter.lt('CLOUD_COVER', 10));</code>

<p>Now you try filtering Landsat data for cloud cover. Filter <code>mayImages</code> to keep only images with less than 5% cloud cover. Store the result in a variable named <code>cleanImages</code>.</p>
<!-- HINT Row -->
<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
...
<p>If your ImageCollection has overlapping scenes, and most of them will, you can combine them into one image using <code>.mosaic()</code>. This makes the resulting composite image easier to display and analyze.</p>

<p>This turns the 31 images in your cleanImages collection, which came from your "mayImages" collection which you then filtered into "cleanImages" collection with clouds <5%, into one mosaic image. </p>
<code>var mosaicImage = cleanImages.mosaic();</code>
...
<p>You can add an image to the map in the Google Earth Engine console using <code>Map.addLayer()</code>. The fourth argument in the "addLayer() function determines whether 
  your shows up immediately in the bottom half of your Google Earth Engine console. If you set the fourth argument to <code>false</code>, the layer is added to the Layers panel but not turned on or visualized by default. This can be helpful if you're working with a global dataset that takes time to render, and you don't want to wait for the entire map to load as an image.</p>
