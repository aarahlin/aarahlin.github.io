...
To make a box around St. Louis, you'll need to provide two corners in the format <code>[west, south, east, north]</code>. A good starting point is a lower-left corner around <code>[-93.0, 36.0]</code>, where -93.0 is the longitude (west) and 36.0 is the latitude (south).

    You can use Google Maps to find coordinates: right-click anywhere on the map and select "What's here?". The coordinates will appear at the bottom. 
    These are usually listed as (latitude, longitude), so be sure to reverse them when entering values in Earth Engine. Coordinates from Google Maps 
    often include multiple decimal places (e.g., -89.6016, 38.6271). You can use those to define a precise bounding box; Google Earth Engine will accept
    whatever level of precision for the coordinates that you provide.
</div>
