...
<h2>Lesson 2 fhfhgfjfhgggfghgggggggg— Working with ImageCollections</h2>
...
<p>Let's load a new image collection dataset into the Google Earth Engin console. You can do this by using the searchbar at the top of the page, or directly loading in the dataset using its name and filepath. For this lesson, we’re using the following dataset:</p>
<code>'LANDSAT/LC08/C02/T1_L2'</code>

<p>The image collection dataset we're going to use is of Landsat 8 data. These images of the Earth's surface have already been atmospherically corrected. Just like the name "image collection" implies, the Landset 8 dataset has thousands of images stacked together into one dataset. Each image represents a single scene from a single day of data collection.</p>
  
<p>Image collections are created using the command</p> <code>ee.ImageCollection()</code>. To load satellite data, use <code>ee.ImageCollection()</code> and put the dataset name in quotes.</p>
  
This particular Landsat data has surface reflectance data, which shows how different surfaces on Earth reflect sunlight in visible, near-infrared, and shortwave infrared spectra, which is useful for identifying vegetation, water, and other kinds of land cover. The dataset also includes surface temperature, and quality control data that we'll use to filter the dataset to retain images with minimal cloud cover. Why would it be helpful to filter out clouds? </p>

<p>Let's create a variable called "landsat_images" that stores our Landsat image collection. Think about how you created variables in lesson 1. Here's some dummy code to help you out. </p>
<code>var my_variable_name = ee.ImageCollection("path/to/dataset");</code>
...
<p> There are way too many images to print all of them. What if you just wanted to print the first image in your stack of Landsat 8 images? Google Earth Engine lets you print the first image in a collection using dot notation like we learned in lesson 1: <code> print(your_image_collection.first(); </code>. The <code> .first() </code> command lets you peek into your image collection's metadata. You can use <code> .first() </code> to check your dataset's structure and make sure it's organized and behaving how you would expect. </p>

<p> Try printing the first image of "landsat_images".</p>
<!-- CODE Row -->
<div style="display:flex; justify-content:space-between; align-items:center;">
