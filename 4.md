...
<p>Let's load a new image collection dataset into the Google Earth Engine console. You can do this by using the search bar at the top of the page, or directly loading in the dataset using its name and filepath. For this lesson, we’re going to use the Landsat 8 image collection dataset:
...
<p>The <code> .first() </code> command lets you peek into your image collection's metadata. You can use <code> .first() </code> to check your dataset's structure and make sure it's organized and behaving how you would expect. Try printing the first image of "landsat_images".</p>
<!-- CODE Row -->
<div style="display:flex; justify-content:space-between; align-items:center;">
