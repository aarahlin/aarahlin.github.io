...
<p>Bonus question, how did I know to select the ST_B10 band to represent land surface temperature in this dataset? </p>
...
<p>Here's an example of how to use ee.Filter.lt():
<code>myImageCollection.filter(ee.Filter.lt('CLOUD_COVER', 10));</code>
</p>

<p>Go ahead and filter your image collection <code>may_images</code> to keep only images with less than 5% cloud cover. Store the result in a new variable called <code>cloudless_images</code>.</p>
<!-- HINT Row -->
<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
...
Use <code>.filter()</code> and <code>ee.Filter.lt()</code> on <code>may_images</code> to filter your images to only those with cloud cover less than 5%.
  </div>
...
<div style="display:flex; justify-content:space-between; align-items:center;">
  <div id="code26" style="visibility:hidden; height:auto; background-color:#f0f0f0; border-left:4px solid #ccc; padding:2px 6px; margin:0px; flex:1;">
    <code>var cloudless_images = may_images.filter(ee.Filter.lt('CLOUD_COVER', 5));</code>
  </div>
...
<p>So far, every time we've mapped the Landsat 8 data, it's been showing us *one* image on top of our pile of images. In our case, it's showing us only May 1, 2024. Our image is choppy and you can see the Landsat satellite tracks, and missing data. Wouldn't it be better to merge all the images in our stack? The <code>.mosaic()</code> function goes through your image collection in order and, for each pixel, takes the value from the first image that has data (non-null) at that location. Basically it takes overlapping scenes in your ImageCollection and combines them into one image, which is useful for visualization.</p>
...
<p>Print <code>my_mosaic</code> to make sure the mosaic command worked as expected. When you look at the data structure in the console, does it look like we only have one image?</p>
...
<div style="display:flex; justify-content:space-between; align-items:center;">
  <div id="code28" style="visibility:hidden; height:auto; background-color:#f0f0f0; border-left:4px solid #ccc; padding:2px 6px; margin:0px; flex:1;">
    <code>print('My mosaic Landsat image:', my_mosaic);</code>
  </div>
...
<div style="display:flex; justify-content:space-between; align-items:center;">
  <div id="code29" style="visibility:hidden; height:auto; background-color:#f0f0f0; border-left:4px solid #ccc; padding:2px 6px; margin:0px; flex:1;">
    var missouriBox = ee.Geometry.Rectangle([-93.5123, 36.2114, -89.1032, 40.3299]); //creates your rectangle that you will use to crop my_mosaic <br>var clipped_mosaic = my_mosaic.clip(missouriBox); //clips my_mosaic to missouriBox<br>Map.addLayer(clipped_mosaic, visualization, 'Clipped May 2024 Mosaic'); //shows clipped_mosaic on the map <br>Map.centerObject(missouriBox, 6); //zooms out </code>
  </div>
...
<p>Navigate to the "Tasks" tab in the upper right corner of your Google Earth Engine console. You'll see your file export pending. </p>
